(ns iea-stats.main
  "Kotodama WASM entrypoint for the iea_stats clean-room actor (L4) — Clojure port.
  ...
  facts (`iea_stats.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "iea_stats")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"iea_stats.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "iea_stats-compat" :tier tier :entities entities} 200])
...
(defn start! [] :iea_stats-compat/ready)
